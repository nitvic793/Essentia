...
float4 main(VertexToPixel input) : SV_TARGET
{
    ...
    float2 velocity = VelocityTexture.Sample(LinearClampSampler, input.uv).xy;
	velocity *= VelocityScale;

	float speed = length(velocity / texelSize);
	int nSamples = clamp(int(speed), 1, MAX_SAMPLES);
    float3 result = InputTexture.Sample(LinearClampSampler, input.uv).rgb;
	[unroll]
	for (int i = 1; i < nSamples; ++i)
	{
		float2 offset = velocity * (float(i) / float(nSamples - 1) - 0.5);
		float2 UV = clamp(offset + input.uv, float2(0.f, 0.f), float2(1.f, 1.f));
        result += InputTexture.SampleLevel(LinearClampSampler, UV, 0).rgb;
    }
