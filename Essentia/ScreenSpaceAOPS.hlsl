...
float3 WorldPosFromDepth(float2 uv, float depth)
{
    ...
    float4 worldSpacePosition = mul(vPositionVS, InvView);
...
float3 ReconstructNormalV2(float2 uv)
{
    ...
    float2 uv1 = uv + float2(1, 0) / float2(1920.f, 1080.f); // right 
    float2 uv2 = uv + float2(0, 1) / float2(1920.f, 1080.f); // top

    float depth0 = DepthTexture.SampleLevel(LinearWrapSampler, uv0, 0).r;
    float depth1 = DepthTexture.SampleLevel(LinearWrapSampler, uv1, 0).r;
    float depth2 = DepthTexture.SampleLevel(LinearWrapSampler, uv2, 0).r;

    float3 P0 = WorldPosFromDepth(uv0, depth0);
    float3 P1 = WorldPosFromDepth(uv1, depth1);
    float3 P2 = WorldPosFromDepth(uv2, depth2);
...
float4 main(VertexOut input) : SV_TARGET
{
    ...
    float depth = DepthTexture.SampleLevel(PointClampSampler, input.UV, 0.f).r;
    float3 vsPos = VSPositionFromDepth(input.UV, depth);
    //float3 normal = ReconstructNormalV2(input.UV);
